/* Segment — shared nav + footer.
   Edit the markup here once and it updates on every page.
   Each page just needs <div id="site-nav"></div>, <div id="site-footer"></div>,
   and <script src="[../]js/site.js"></script> before </body>.
   Links are root-relative clean paths (/projects, /contact, ...). The .html
   files still exist; netlify.toml redirects them to these clean paths. */
(function () {
  // Normalise the current path: drop a trailing slash and any .html extension,
  // so /projects, /projects/, /projects.html all reduce to the same thing.
  var path = location.pathname.replace(/\/+$/, '').replace(/\.html$/, '');
  var last = path.split('/').pop(); // '', 'projects', 'contact', 'allday', ...

  // Work out which nav item should be marked active.
  var active = '';
  if (path.indexOf('/projects') === 0) active = 'projects'; // listing + detail pages
  else if (last.indexOf('fieldnotes') === 0) active = 'fieldnotes';
  else if (last === 'team') active = 'team';
  else if (last === 'contact') active = 'contact';
  else if (last === '' || last === 'index') active = 'home';

  function navLink(href, label, key) {
    var cls = 'nav__link' + (active === key ? ' nav__link--active' : '');
    return '<li><a href="' + href + '" class="' + cls + '">' + label + '</a></li>';
  }

  var navHTML =
    '<nav class="nav" id="nav">' +
      '<div class="nav__inner">' +
        '<a href="/" class="nav__logo"><span>Segment</span></a>' +
        '<button class="nav__toggle" aria-label="Menu" onclick="document.querySelector(\'.nav__links\').classList.toggle(\'open\')">' +
          '<span></span><span></span><span></span>' +
        '</button>' +
        '<ul class="nav__links">' +
          navLink('/', 'Home', 'home') +
          navLink('/projects', 'Projects', 'projects') +
          navLink('/fieldnotes', 'Fieldnotes', 'fieldnotes') +
          navLink('/team', 'Team', 'team') +
          navLink('/contact', 'Contact', 'contact') +
          '<li><a href="/contact" class="nav__cta">Get in touch</a></li>' +
        '</ul>' +
      '</div>' +
    '</nav>';

  var footerHTML =
    '<footer class="footer">' +
      '<div class="container">' +
        '<div class="footer__inner">' +
          '<a href="/" class="footer__logo">Segment</a>' +
          '<ul class="footer__links">' +
            '<li><a href="/" class="footer__link">Home</a></li>' +
            '<li><a href="/projects" class="footer__link">Projects</a></li>' +
            '<li><a href="/fieldnotes" class="footer__link">Fieldnotes</a></li>' +
            '<li><a href="/team" class="footer__link">Team</a></li>' +
            '<li><a href="/contact" class="footer__link">Contact</a></li>' +
            '<li><a href="https://www.linkedin.com/in/sophie-lucassen/" class="footer__link" target="_blank">LinkedIn</a></li>' +
          '</ul>' +
          '<p class="footer__copy">Segment \u00B7 Fractional marketing \u00B7 Guernsey \u00A9 2026</p>' +
        '</div>' +
      '</div>' +
    '</footer>';

  function inject() {
    var navMount = document.getElementById('site-nav');
    var footMount = document.getElementById('site-footer');
    if (navMount) navMount.outerHTML = navHTML;
    if (footMount) footMount.outerHTML = footerHTML;

    var nav = document.getElementById('nav');
    if (nav) {
      var onScroll = function () {
        nav.classList.toggle('scrolled', window.scrollY > 40);
      };
      onScroll();
      window.addEventListener('scroll', onScroll, { passive: true });
    }
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', inject);
  } else {
    inject();
  }
})();
